...
37. **Theme-compatible backgrounds via cascading var().** For section components with colored backgrounds, use `var(--gs-{component}-bg, var(--gs-color-primary-bg))`. The inner `var()` cascades: consumers override `--gs-{component}-bg` for custom colors, otherwise it falls back to `--gs-color-primary-bg` which automatically adapts to any theme palette (orange → peach, blue → light blue, gray → light gray, dark → dark surface). Never hardcode gradients.

38. **Content alignment across page sections.** When a section component (SaleHits, BannerCarousel, etc.) lives inside a page layout with `max-width` + `padding`, the section's internal content must align horizontally with sibling sections (header, product grid). Use `padding: vertical 0` on the section root, add horizontal padding only to header/controls, let scrollable content go edge-to-edge within the rounded card.

39. **Rich stories: 8+ per component covering all dimensions.** Every component story file should have 8+ stories covering: all prop variants, all sizes, edge cases (empty/long text/max values), interactive controlled state, locales, and realistic e-commerce data. Genuinely different data per story — not copy-paste with different names. Scale via parallel agents (10 agents, ~168 files, ~1700 stories in one session).
