...
describe('DesktopCategoryShowcase', () => {
  it('renders all category labels', () => {
    render(<DesktopCategoryShowcase categories={categories} />);
  ...
  it('does not render item count when not provided', () => {
    ...
    render(<DesktopCategoryShowcase categories={noCounts} />);
    expect(screen.queryByText(/items/)).not.toBeInTheDocument();
  });

  it('renders section title when provided', () => {
    render(<DesktopCategoryShowcase categories={categories} title="Shop by Category" />);
  ...
  it('renders as links when href is provided', () => {
    ...
    render(<DesktopCategoryShowcase categories={withLinks} />);
  ...
  it('calls onClick when category card is clicked', async () => {
    ...
    render(<DesktopCategoryShowcase categories={clickable} />);
  ...
  it('sets grid columns from columns prop', () => {
    const { container } = render(<DesktopCategoryShowcase categories={categories} columns={4} />);
    const grid = container.querySelector('[class*="grid"]') as HTMLElement;
    expect(grid.style.gridTemplateColumns).toBe('repeat(4, 1fr)');
  });

  it('defaults to 5 columns', () => {
    const { container } = render(<DesktopCategoryShowcase categories={categories} />);
    const grid = container.querySelector('[class*="grid"]') as HTMLElement;
  ...
  it('applies custom className', () => {
    const { container } = render(
      <DesktopCategoryShowcase categories={categories} className="my-showcase" />
    );
