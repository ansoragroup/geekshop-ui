...
export const DesktopTelegramLoginButton = forwardRef<
  HTMLButtonElement,
...
>(
  (
    ...
    // Validate botName to prevent injection via URL interpolation
    const isValidBotUsername = /^[a-zA-Z0-9_]{5,32}$/.test(botName);
    if (!isValidBotUsername) {
      console.error(
        `[DesktopTelegramLoginButton] Invalid botName "${botName}". Must be 5-32 alphanumeric/underscore characters.`
      );
      return null;
    }
    ...
    const mergedStyle =
      cornerRadius !== undefined ? { ...style, borderRadius: cornerRadius } : style;
