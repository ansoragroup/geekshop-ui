...
export const TelegramLoginButton = forwardRef<HTMLButtonElement, TelegramLoginButtonProps>(
  (
    ...
    const handleClick = useCallback(() => {
      ...
      if (callbackUrl) {
        window.open(
          `https://oauth.telegram.org/auth?bot_id=${botUsername}&origin=${encodeURIComponent(
            callbackUrl
          )}`,
          '_blank',
          'noopener,noreferrer'
    ...
    // Validate botUsername to prevent injection via data attributes / URL interpolation
    const isValidBotUsername = /^[a-zA-Z0-9_]{5,32}$/.test(botUsername);
    if (!isValidBotUsername) {
      console.error(
        `[TelegramLoginButton] Invalid botUsername "${botUsername}". Must be 5-32 alphanumeric/underscore characters.`
      );
      return null;
    }
    ...
    if (variant === 'widget') {
      return (
        <div
          ...
          className={cn(styles.widgetContainer, className)}
