...
const CategorySidebar = () => (
  <div style={{ background: '#fff', borderRadius: 8, padding: 16 }}>
    ...
    {['Noutbuklar', 'Videokartalar', 'Protsessorlar', 'Monitorlar', 'Klaviaturalar', 'Sichqonchalar', 'Quloqchinlar', 'SSD & HDD'].map((cat) => (
      <div key={cat} style={{ padding: '10px 0', fontSize: 13, color: '#666', borderBottom: '1px solid #f0f0f0', cursor: 'pointer' }}>
        {cat}
      </div>
    ))}
  </div>
...
const AccountSidebar = () => (
  <div style={{ background: '#fff', borderRadius: 8, padding: 16 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
      <div style={{ width: 48, height: 48, borderRadius: '50%', background: '#FF5000', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 18 }}>A</div>
      <div>
        <div style={{ fontWeight: 600, fontSize: 14 }}>Alisher Navro'zov</div>
        <div style={{ fontSize: 12, color: '#999' }}>alisher@email.com</div>
      </div>
    </div>
    {['Buyurtmalar', 'Manzillar', 'To\'lov usullari', 'Kuponlar', 'Sevimlilar', 'Sozlamalar', 'Chiqish'].map((item, i) => (
      <div key={item} style={{ padding: '10px 0', fontSize: 13, color: i === 6 ? '#FF3B30' : '#666', borderBottom: '1px solid #f0f0f0', cursor: 'pointer' }}>
        {item}
      </div>
    ))}
  </div>
...
const OrderContent = () => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
    {[
      { id: '#GS-20261001', date: '24.03.2026', status: 'Yetkazildi', statusColor: '#07C160', total: '8 900 000' },
      { id: '#GS-20260998', date: '22.03.2026', status: 'Yo\'lda', statusColor: '#1890FF', total: '2 400 000' },
      { id: '#GS-20260945', date: '18.03.2026', status: 'Bekor qilindi', statusColor: '#FF3B30', total: '15 200 000' },
    ].map((order) => (
      <div key={order.id} style={{ background: '#fff', borderRadius: 8, padding: 16, border: '1px solid #eee' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 13, color: order.statusColor, fontWeight: 500 }}>{order.status}</span>
          <span style={{ fontSize: 14, fontWeight: 700, color: '#1A1A1A' }}>{order.total} so'm</span>
        </div>
      </div>
    ))}
  </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 8, padding: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8, fontSize: 13 }}>
            <input type="text" placeholder="dan" style={{ flex: 1, padding: '6px 8px', border: '1px solid #eee', borderRadius: 4, fontSize: 13 }} />
            <input type="text" placeholder="gacha" style={{ flex: 1, padding: '6px 8px', border: '1px solid #eee', borderRadius: 4, fontSize: 13 }} />
          </div>
        </div>
      </div>
    ),
