      ...
      <div style={{ padding: '24px 16px', textAlign: 'center' }}>
        <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#E8F8EF', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
          <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
            <path d="M7 14L12 19L21 9" stroke="#07C160" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </div>
        <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Buyurtma qabul qilindi!</div>
        <div style={{ fontSize: 13, color: '#666' }}>Buyurtma #GS-2026-0092 muvaffaqiyatli rasmiylashtirildi.</div>
      </div>
    ),
  },
};
    ...
    children: (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {[
          ...
          <div key={opt.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', background: '#F5F5F5', borderRadius: 8 }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 500 }}>{opt.label}</div>
              <div style={{ fontSize: 12, color: '#999', marginTop: 2 }}>{opt.time}</div>
            </div>
            <div style={{ fontSize: 14, fontWeight: 600, color: opt.price === 'Bepul' ? '#07C160' : '#FF5000' }}>{opt.price}</div>
          </div>
        ))}
      </div>
    ),
  ...
  args: {
    ...
    children: (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {[
          { name: 'Aziz K.', date: '15 Mar 2026', rating: 5, text: 'Juda zo\'r mahsulot! Tez yetkazib berildi, sifati a\'lo darajada. Tavsiya qilaman.' },
          { name: 'Madina R.', date: '12 Mar 2026', rating: 4, text: 'Yaxshi mahsulot. Narxiga arziydi. Faqat quti biroz ezilgan edi.' },
          { name: 'Sardor B.', date: '10 Mar 2026', rating: 5, text: 'Kutganimdan ham yaxshiroq chiqdi. O\'yin o\'ynash uchun zo\'r.' },
          { name: 'Nilufar T.', date: '8 Mar 2026', rating: 3, text: 'O\'rtacha. Rasm bilan farq qiladi. Lekin ishlaydi.' },
          { name: 'Jamshid M.', date: '5 Mar 2026', rating: 5, text: 'Mukammal! Ikkinchi marta xarid qilyapman. Sifatga ishonaman.' },
          { name: 'Dilfuza A.', date: '2 Mar 2026', rating: 4, text: 'Yetkazib berish tez bo\'ldi, mahsulot sifatli. Rahmat!' },
          { name: 'Ulugbek S.', date: '28 Feb 2026', rating: 5, text: 'Zo\'r narx. Boshqa do\'konlarda bundan qimmat.' },
          { name: 'Gulnora P.', date: '25 Feb 2026', rating: 2, text: 'Yetkazib berish juda uzoq davom etdi. Mahsulot yaxshi lekin servis yomon.' },
        ].map((review, i) => (
          <div key={i} style={{ padding: '12px 0', borderBottom: i < 7 ? '1px solid #eee' : 'none' }}>
            ...
            <div style={{ display: 'flex', gap: 2, marginBottom: 6 }}>
              {Array.from({ length: 5 }, (_, j) => (
                <span key={j} style={{ fontSize: 12, color: j < review.rating ? '#FFA726' : '#ddd' }}>&#9733;</span>
              ))}
            </div>
  ...
  render: () => {
    ...
    return (
      <div style={{ padding: 16 }}>
        ...
        <BottomSheet visible={visible} title="Yangi manzil" height="70vh" onClose={() => setVisible(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, paddingBottom: 16 }}>
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#333' }}>To'liq ism</label>
              <input type="text" placeholder="Ismingizni kiriting" style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #eee', fontSize: 14, boxSizing: 'border-box' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#333' }}>Telefon raqam</label>
              <input type="tel" placeholder="+998 90 123 45 67" style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #eee', fontSize: 14, boxSizing: 'border-box' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#333' }}>Shahar</label>
              <select style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #eee', fontSize: 14, boxSizing: 'border-box', background: '#fff' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#333' }}>Manzil</label>
              <textarea rows={3} placeholder="Ko'cha, uy raqami, kvartira" style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #eee', fontSize: 14, boxSizing: 'border-box', resize: 'vertical' }} />
            </div>
            <button style={{ padding: '14px', borderRadius: 12, border: 'none', background: '#FF5000', color: '#fff', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>
              Saqlash
            </button>
          </div>
        </BottomSheet>
  ...
  args: {
    ...
    children: (
      <div style={{ textAlign: 'center', padding: '40px 16px' }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 600, color: '#333', marginBottom: 6 }}>Bildirishnomalar yo'q</div>
        <div style={{ fontSize: 13, color: '#999' }}>Yangi bildirishnomalar paydo bo'lganda bu yerda ko'rinadi</div>
      </div>
    ),
  },
};
  ...
  render: () => {
    ...
    return (
      <div style={{ padding: 16 }}>
        <button
          onClick={() => setVisible(true)}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: '#fff', borderRadius: 20, border: '1px solid #eee', cursor: 'pointer', fontSize: 13, color: '#333' }}
        >
        ...
        <BottomSheet visible={visible} title="Saralash" height="45vh" onClose={() => setVisible(false)}>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {options.map((opt) => (
              <div
                key={opt.key}
                onClick={() => { setSelected(opt.key); setVisible(false); }}
                onKeyDown={(e) => { if (e.key === 'Enter') { setSelected(opt.key); setVisible(false); } }}
                role="button"
                tabIndex={0}
                style={{
                  padding: '14px 0',
              ...
              >
                ...
                {selected === opt.key && (
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    <path d="M3 8l4 4 6-7" stroke="#FF5000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                )}
              </div>
            ))}
  ...
  args: {
    ...
    children: (
      <div style={{ fontSize: 14, lineHeight: 1.7, color: '#666' }}>
        <h4 style={{ color: '#333', margin: '0 0 12px' }}>NVIDIA GeForce RTX 4090 Founders Edition</h4>
        <p style={{ margin: '0 0 12px' }}>
          NVIDIA GeForce RTX 4090 — bu dunyodagi eng kuchli iste'molchi grafik kartasi. Ada Lovelace arxitekturasi asosida qurilgan bu karta, avvalgi avlodga nisbatan 2-4 baravar yuqori unumdorlikni ta'minlaydi.
        </p>
          ...
          <tbody>
            {[
              ...
              <tr key={label} style={{ borderBottom: '1px solid #f0f0f0' }}>
                ...
                <td style={{ padding: '8px 0', textAlign: 'right', fontWeight: 500, color: '#333' }}>{value}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <p style={{ margin: 0 }}>3 yil ishlab chiqaruvchi kafolati. GeekShop orqali 14 kun ichida qaytarish mumkin.</p>
      </div>
    ),
  },
};
