...
export const Disabled: Story = {
  args: {
    ...
    children: (
      <div style={{ padding: '16px', background: '#f9f9f9', borderBottom: '1px solid #eee', opacity: 0.6 }}>
...
export const NotificationList: Story = {
  ...
  render: () => {
    const [notifications, setNotifications] = useState([
      { id: 1, icon: '\u{1F4E6}', title: 'Buyurtmangiz yo\'lda', text: '#GS-2026-0091 yetkazib berilmoqda', time: '2 daqiqa oldin', unread: true },
      { id: 2, icon: '\u{1F3F7}', title: 'Flash chegirma!', text: 'RTX 4070 Super — 30% chegirma', time: '15 daqiqa oldin', unread: true },
      { id: 3, icon: '\u2705', title: 'To\'lov tasdiqlandi', text: '#GS-2026-0088 uchun to\'lov qabul qilindi', time: '1 soat oldin', unread: false },
      { id: 4, icon: '\u{1F4AC}', title: 'Yangi javob', text: 'Sotuvchi savolingizga javob berdi', time: '3 soat oldin', unread: false },
    ]);
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 1, background: '#f5f5f5' }}>
        {notifications.map((n) => (
          ...
          >
            <div style={{ display: 'flex', gap: 12, padding: '14px 16px', background: n.unread ? '#FFF5F0' : '#fff' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 11, color: '#999', whiteSpace: 'nowrap' }}>{n.time}</span>
                </div>
                <div style={{ fontSize: 13, color: '#666', marginTop: 2 }}>{n.text}</div>
              </div>
              {n.unread && <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#FF5000', flexShrink: 0, marginTop: 6 }} />}
            </div>
          </Swipe>
        ))}
...
export const OrderList: Story = {
  ...
  render: () => {
    const orders = [
      { id: 'GS-2026-0093', date: '25 Mar 2026', status: 'Yetkazildi', statusColor: '#07C160', total: '24 990 000' },
      { id: 'GS-2026-0088', date: '20 Mar 2026', status: 'Yo\'lda', statusColor: '#1890FF', total: '7 490 000' },
      { id: 'GS-2026-0075', date: '15 Mar 2026', status: 'Tayyorlanmoqda', statusColor: '#FFA726', total: '12 500 000' },
    ];
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 1, background: '#f5f5f5' }}>
        {items.map((item) => (
          ...
          >
            <div style={{ display: 'flex', gap: 12, padding: '14px 16px', background: '#fff', alignItems: 'center' }}>
              <div style={{ width: 56, height: 56, borderRadius: 8, background: '#f0f0f0', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <path d="M12 6c-2.5-3.5-7-3.5-8.5 0S5.5 15 12 21c6.5-6 8-10 6.5-15S14.5 2.5 12 6z" fill="#FF5000" />
                </svg>
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.name}</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                  <span style={{ fontSize: 14, fontWeight: 600, color: '#FF0000' }}>{item.price} so'm</span>
...
export const AddressManagement: Story = {
  ...
  render: () => {
    const [addresses, setAddresses] = useState([
      { id: 1, label: 'Uy', name: 'Aziz Karimov', address: 'Amir Temur ko\'chasi, 108-uy, 24-xonadon', city: 'Toshkent', isDefault: true },
      { id: 2, label: 'Ofis', name: 'Aziz Karimov', address: 'Mustaqillik ko\'chasi, 55-uy', city: 'Toshkent', isDefault: false },
      { id: 3, label: 'Ota-ona', name: 'Sardor Karimov', address: 'Navoiy ko\'chasi, 12-uy', city: 'Samarqand', isDefault: false },
    ]);
          ...
          >
            <div style={{ padding: '14px 16px', background: '#fff' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                {addr.isDefault && (
                  <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: '#FFF5F0', color: '#FF5000', fontWeight: 500 }}>Asosiy</span>
                )}
              </div>
              <div style={{ fontSize: 13, color: '#666' }}>{addr.name}</div>
              <div style={{ fontSize: 13, color: '#999', marginTop: 2 }}>{addr.address}, {addr.city}</div>
            </div>
          </Swipe>
        ))}
      </div>
