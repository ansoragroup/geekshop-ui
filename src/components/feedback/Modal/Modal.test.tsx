...
describe('Modal', () => {
  ...
  it('renders title in the header', () => {
    render(<Modal open={true} onClose={vi.fn()} title="My Title">Content</Modal>);
  ...
  it('does not render footer when not provided', () => {
    render(
      <Modal open={true} onClose={vi.fn()}>Content</Modal>,
    );
  ...
  it('does not call onClose on backdrop click when closeOnBackdrop is false', async () => {
    ...
    const onClose = vi.fn();
    const { container } = render(
      <Modal open={true} onClose={onClose} closeOnBackdrop={false}>Content</Modal>,
    );
    const backdrop = container.firstElementChild as HTMLElement;
    await user.click(backdrop);
  ...
  it('does not call onClose when content area is clicked', async () => {
    const user = userEvent.setup();
    const onClose = vi.fn();
    render(<Modal open={true} onClose={onClose}>Click me</Modal>);
    await user.click(screen.getByText('Click me'));
    expect(onClose).not.toHaveBeenCalled();
  });

  it('applies custom width as number', () => {
    render(<Modal open={true} onClose={vi.fn()} width={600}>Content</Modal>);
    const dialog = screen.getByRole('dialog');
    expect(dialog.style.width).toBe('600px');
  });

  it('applies custom width as string', () => {
    render(<Modal open={true} onClose={vi.fn()} width="50%">Content</Modal>);
    const dialog = screen.getByRole('dialog');
  ...
  it('forwards ref', () => {
    ...
    render(<Modal ref={ref} open={true} onClose={vi.fn()}>Content</Modal>);
    expect(ref.current).toBeInstanceOf(HTMLDivElement);
  });

  it('spreads rest props onto dialog element', () => {
    render(
      <Modal open={true} onClose={vi.fn()} data-testid="modal">Content</Modal>,
    );
    const dialog = screen.getByRole('dialog');
  ...
  it('merges custom className onto backdrop', () => {
    const { container } = render(
      <Modal open={true} onClose={vi.fn()} className="custom-modal">Content</Modal>,
    );
    const backdrop = container.firstElementChild as HTMLElement;
