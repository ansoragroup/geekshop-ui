...
const FooterButtons = ({ onClose, primaryLabel = 'Apply', secondaryLabel = 'Reset' }: { onClose: () => void; primaryLabel?: string; secondaryLabel?: string }) => (
  <>
    <button
      type="button"
      onClick={onClose}
    ...
    return (
      <div style={{ padding: 24 }}>
        ...
        <DesktopBottomSheet
          ...
          open={open}
          onClose={() => setOpen(false)}
          footer={<FooterButtons onClose={() => setOpen(false)} primaryLabel="Track Order" secondaryLabel="Close" />}
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
    ...
    return (
      <div style={{ padding: 24 }}>
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              { icon: '\u{1F4E6}', text: 'Your order #GS-2026-0091 has been shipped', time: '2 min ago', unread: true },
              { icon: '\u{1F3F7}', text: 'Flash sale starts in 1 hour! Up to 50% off', time: '15 min ago', unread: true },
              { icon: '\u2705', text: 'Payment confirmed for order #GS-2026-0088', time: '1 hour ago', unread: false },
              { icon: '\u{1F4AC}', text: 'Seller replied to your question about RTX 4080', time: '3 hours ago', unread: false },
            ].map((n, i) => (
              <div key={i} style={{ display: 'flex', gap: 12, padding: '12px', borderRadius: 8, background: n.unread ? '#FFF5F0' : '#f9f9f9' }}>
    ...
    return (
      <div style={{ padding: 24 }}>
        ...
        <DesktopBottomSheet
          ...
          open={open}
          onClose={() => setOpen(false)}
          footer={<FooterButtons onClose={() => setOpen(false)} primaryLabel="Save Address" secondaryLabel="Cancel" />}
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', gap: 12 }}>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#333' }}>First Name</label>
                <input type="text" defaultValue="Aziz" style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #ddd', fontSize: 14, boxSizing: 'border-box' }} />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#333' }}>Last Name</label>
                <input type="text" defaultValue="Karimov" style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #ddd', fontSize: 14, boxSizing: 'border-box' }} />
              </div>
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#333' }}>Phone Number</label>
              <input type="tel" defaultValue="+998 90 123 45 67" style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #ddd', fontSize: 14, boxSizing: 'border-box' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#333' }}>City</label>
              <select defaultValue="Tashkent" style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #ddd', fontSize: 14, boxSizing: 'border-box', background: '#fff' }}>
            ...
            <div>
              ...
              <textarea rows={3} defaultValue="Amir Temur ko'chasi, 108-uy, 24-xonadon" style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #ddd', fontSize: 14, boxSizing: 'border-box', resize: 'vertical' }} />
            </div>
          </div>
        </DesktopBottomSheet>
      </div>
    ...
    return (
      <div style={{ padding: 24 }}>
        ...
        <DesktopBottomSheet
          ...
          open={open}
          onClose={() => setOpen(false)}
          footer={<FooterButtons onClose={() => setOpen(false)} primaryLabel="I Agree" secondaryLabel="Cancel" />}
        >
          <div style={{ fontSize: 14, lineHeight: 1.7, color: '#666' }}>
            <h4 style={{ color: '#333', margin: '0 0 8px' }}>1. General Return Policy</h4>
            <p style={{ margin: '0 0 16px' }}>You may return most items within 14 days of delivery for a full refund. Items must be unused, in their original packaging, and with all tags and accessories included.</p>
            <h4 style={{ color: '#333', margin: '0 0 8px' }}>2. Electronics Return Policy</h4>
            <p style={{ margin: '0 0 16px' }}>Electronics (laptops, phones, tablets, GPUs, etc.) may be returned within 7 days of delivery if they have a manufacturing defect. The product must not show signs of physical damage.</p>
            ...
            <p style={{ margin: '0 0 16px' }}>Once we receive your returned item, we will inspect it and process your refund within 5-7 business days. Refunds are issued to the original payment method.</p>
            <h4 style={{ color: '#333', margin: '0 0 8px' }}>5. Exchange Policy</h4>
            <p style={{ margin: '0 0 16px' }}>If you received a defective or incorrect item, we will ship a replacement at no additional cost. Contact our support team to initiate an exchange.</p>
            <h4 style={{ color: '#333', margin: '0 0 8px' }}>6. Shipping Costs</h4>
            <p style={{ margin: 0 }}>Return shipping is free for defective products. For other returns, a flat fee of 25,000 UZS will be deducted from your refund to cover return shipping costs.</p>
          </div>
        </DesktopBottomSheet>
      </div>
    );
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {slots.map((slot) => (
              <div
                ...
                onKeyDown={(e) => { if (e.key === 'Enter' && slot.available) setSelected(slot.key); }}
                role="button"
                ...
                style={{
                  ...
                  background: !slot.available ? '#f9f9f9' : selected === slot.key ? '#FFF5F0' : '#fff',
              ...
              >
                ...
                {!slot.available && <span style={{ fontSize: 12, color: '#FF3B30', fontWeight: 500 }}>Full</span>}
...
export const CartDrawer: Story = {
  ...
  render: (args) => {
    const [open, setOpen] = useState(false);
    const cartItems = [
      { name: 'Logitech G PRO X Superlight 2', variant: 'White', qty: 1, price: '1,890,000', img: 'https://images.unsplash.com/photo-1527864550417-7fd91fc51a46?w=80&h=80&fit=crop' },
      { name: 'SteelSeries Arctis Nova Pro', variant: 'Black', qty: 1, price: '4,200,000', img: 'https://images.unsplash.com/photo-1599669454699-248893623440?w=80&h=80&fit=crop' },
      { name: 'Corsair K70 RGB Pro', variant: 'Cherry MX Red', qty: 2, price: '2,350,000', img: 'https://images.unsplash.com/photo-1618384887929-16ec33fab9ef?w=80&h=80&fit=crop' },
    ];
          ...
          footer={
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
              ...
              <button
                ...
                style={{ padding: '12px 32px', borderRadius: 8, border: 'none', background: '#FF5000', color: '#fff', cursor: 'pointer', fontSize: 14, fontWeight: 600 }}
              >
                Checkout
              </button>
            </div>
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {cartItems.map((item, i) => (
              <div key={i} style={{ display: 'flex', gap: 12, padding: '12px', background: '#f9f9f9', borderRadius: 8 }}>
                <img src={item.img} alt={item.name} style={{ width: 72, height: 72, borderRadius: 8, objectFit: 'cover', flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.name}</div>
                  <div style={{ fontSize: 12, color: '#999', marginTop: 2 }}>{item.variant} | Qty: {item.qty}</div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#FF0000', marginTop: 6 }}>{item.price} UZS</div>
                </div>
              </div>
            ))}
          </div>
        ...
        >
          <div style={{ textAlign: 'center', padding: '48px 24px' }}>
            <svg width="64" height="64" viewBox="0 0 64 64" fill="none" style={{ marginBottom: 16 }}>
              ...
              <path d="M32 22c-3.5-5-10-5-12 0s2 12 12 18c10-6 14-13 12-18s-8.5-5-12 0z" stroke="#ddd" strokeWidth="2" fill="none" />
            </svg>
            <h4 style={{ margin: '0 0 8px', fontSize: 16, fontWeight: 600, color: '#333' }}>Your wishlist is empty</h4>
            <p style={{ margin: 0, fontSize: 14, color: '#999' }}>Save items you love to find them later.</p>
          </div>
        </DesktopBottomSheet>
      </div>
    );
