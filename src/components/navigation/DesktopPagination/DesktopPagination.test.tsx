...
describe('DesktopPagination', () => {
  it('renders as a nav element with correct aria-label', () => {
    render(<DesktopPagination currentPage={1} totalPages={5} onPageChange={vi.fn()} />);
  ...
  it('marks current page with aria-current', () => {
    render(<DesktopPagination currentPage={3} totalPages={5} onPageChange={vi.fn()} />);
  ...
  it('calls onPageChange when a page button is clicked', () => {
    ...
    render(<DesktopPagination currentPage={1} totalPages={5} onPageChange={onPageChange} />);
  ...
  it('disables Next button on last page', () => {
    render(<DesktopPagination currentPage={5} totalPages={5} onPageChange={vi.fn()} />);
    expect(screen.getByLabelText('Next page')).toBeDisabled();
  });

  it('calls onPageChange with previous page when Prev is clicked', () => {
    const onPageChange = vi.fn();
    render(<DesktopPagination currentPage={3} totalPages={5} onPageChange={onPageChange} />);
  ...
  it('shows ellipsis for large page counts', () => {
    render(<DesktopPagination currentPage={6} totalPages={20} onPageChange={vi.fn()} />);
    const ellipses = screen.getAllByText('...');
    expect(ellipses.length).toBeGreaterThanOrEqual(1);
  });

  it('always shows first and last pages', () => {
    render(<DesktopPagination currentPage={10} totalPages={20} onPageChange={vi.fn()} />);
  ...
  it('renders nothing when totalPages is 0', () => {
    const { container } = render(
      <DesktopPagination currentPage={1} totalPages={0} onPageChange={vi.fn()} />
    );
  ...
  it('applies custom className', () => {
    const { container } = render(
      <DesktopPagination currentPage={1} totalPages={5} onPageChange={vi.fn()} className="custom" />
    );
