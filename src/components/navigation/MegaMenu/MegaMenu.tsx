...
export const MegaMenu = forwardRef<HTMLElement, MegaMenuProps>(
  ({ categories, navItems, onCategoryClick, onSubcategoryClick, triggerLabel, triggerIcon, viewAllLabel = 'View all', className, ...rest }, ref) => {
    const { t } = useGeekShop();
    ...
    return (
      <nav ref={ref} className={rootClass} aria-label={t('aria.categoryNavigation')} {...rest}>
        ...
        {isOpen && categories.length > 0 && (
          <div ref={panelRef} className={styles.panel} role="menu" aria-label={t('aria.categories')} tabIndex={-1}>
            {/* Category list (left) */}
                ...
                >
                  ...
                  >
                    ...
                    {cat.subcategories && cat.subcategories.length > 0 && (
                      <span className={styles.chevron}><ArrowRightIcon /></span>
                    )}
                  </a>
                </li>
              ))}
